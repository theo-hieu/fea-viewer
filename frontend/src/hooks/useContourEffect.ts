 ...
 */
export function useContourEffect(
  ...
  // Track previous timestep to prevent infinite loops from React state changes
  ...
  useEffect(() => {
    ...
    // Prevent infinite fetch loops if React re-runs the effect due to state changes
    if (
    ...
    const loadField = async () => {
      try {
        ...
        const values_f64 = decodeTypedArray(
          res.buffer,
          res.meta.dtype,
        ) as Float64Array;
        ...
        if (contourMat) {
          contourManager.swapMeshMaterials(
            meshManager.getMeshGroups(),
            contourMat,
          );
        }
  ...
  }, [colorMapConfig.lut_name, contourManager, activeFieldId]);
}
