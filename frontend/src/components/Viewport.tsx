...
export const Viewport: React.FC<ViewportProps> = ({
  ...
  useContourEffect(contourManagerRef.current, meshManagerRef.current);

  useDeformEffect(
    deformManagerRef.current,
    meshManagerRef.current,
    wireframeManagerRef.current,
    contourManagerRef.current,
    sceneManagerRef.current?.scene ?? null,
  );
  ...
  useEffect(() => {
    ...
    if (perf.memory) {
      ...
      if (usedMB > 2048) {
        ...
        console.warn(
          "[Memory] Heap approaching 1.5GB - performance may degrade",
        );
      }
    }
  }, [status]);
    ...
    return (
      <div className="webgl-fallback">
        ...
        <div className="webgl-fallback__text">
          Your browser does not support WebGL2. The 3D viewer requires WebGL2
          for rendering. You can still view model metadata and text-based
          summaries.
        </div>
      </div>
    );
  ...
  const pickSceneBuiltRef = useRef<{ mode: string; gen: number }>({
    mode: "",
    gen: 0,
  });
  ...
  useEffect(() => {
    ...
    if (
      ...
      pickSceneBuiltRef.current.gen === key.gen
    )
      return;
